    ...
    .then(() => {
        ...
        server.post('/save', (req, res) => {
            MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
                ...
                //Replace chromes %20 with a space for database store. 
                ...
                //Create object to be inserted into the database 
                var myobj = {
                    BOOKING: array[0],
            ...
            //Return to the updated application state
        ...
        server.post("/find", function (req, res) {
            ...
            MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
                ...
                dbo.collection("records").find(query).toArray(function (err, result) {
                    if (err) throw err;
                    if (result.length == 0) {
                        //Redirect user to empty state if they don't exist 
                        res.redirect("../app");
                    } else {
                        resultValuesArray = Object.values(result[0]);
                        db.close();

                        //Create return URL
                        var i;
                        for (i = 1; i < resultValuesArray.length; i++) {
                            resultURL += resultValuesArray[i] + ",";
                        }

                        //Append ID to URL
                        resultURL += resultValuesArray[0];
        ...
        server.post('/teams', function (req, res) {
            MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
                ...
                //Only return the teams from the database
